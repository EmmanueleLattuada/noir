...
use super::{
    persistency_service::PersistencyService, redis_handler::RedisHandler,
    state_saver::StateSaverHandler, PersistencyServices,
};
...
impl PersistencyBuilder {
    ...
    pub(super) fn compute_last_complete_snapshot(
        &mut self,
        operators: Vec<OperatorCoord>,
    ...
    fn compute_last_iter_stack(
        &mut self,
        snap_index: u64,
        op_iter_indexes: HashMap<u64, Vec<OperatorCoord>>,
    ) {
        // each iter index
    ...
    pub(crate) fn clean_persisted_state(&mut self, operators: Vec<OperatorCoord>) {
        for op_coord in operators {
            ...
            while last_opt_snap.is_some() {
                self.handler
                    .delete_state(&op_coord, &last_opt_snap.unwrap());
    ...
    pub(crate) fn generate_persistency_service<State: ExchangeData>(
        &self,
    ) -> PersistencyService<State> {
